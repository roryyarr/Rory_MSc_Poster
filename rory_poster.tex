...
{\IfFileExists{UoM_logo.png}{\includegraphics[height=6em]{UoM_logo.png}}{\rule{0pt}{6em}\hspace{7.5em}}}
{\bf\textsc{The Gunk $\cG$-Transform on the Sphere}\vspace{0.28em}\\[-0.15em]\large Integral geometry, spherical harmonics and inversion}
...
In 1911 Paul Funk introduced the following transform over the sphere. For $\omega\in\Sph^2$ and $f\in C^\infty(\Sph^2)$ let
\vspace{-0.9em}
...
% =============================================================================
\headerbox{\textcolor{white}{2.} The Gunk $\cG$-transform: a normal derivative along great circles}{name=ggeom,column=1,span=2,row=0}{

\begin{center}
\fcolorbox{blue}{softblue}{\begin{minipage}{0.95\linewidth}\centering
\textbf{Definition.} For $f\in C^1(\Sph^2)$ and $\omega\in\Sph^2$, the \emph{Gunk} transform is
\vspace{-0.3em}
...
\vspace{-0.8em}
\[\quad\cG[R\cdot f]=R\cdot\cG[f]\quad (R\in\SO(3)), \qquad (R\cdot f)(\sigma)=f(R^{-1}\sigma). \vspace{-0.4em} \]
Hence, every output of $\cG$ is odd, and $\cG$ commutes with rotations.
}
...
Since $\cF,\cG$ and $\Delta$ are $\SO(3)$-equivariant, Schur's lemma says $\cF\big|_{\cH_\ell}=\lambda_\ell'I$ and $\cG\big|_{\cH_\ell}=\lambda_\ell I$. Hence, we can diagonalise our transforms by
\vspace{-0.4em}
\begin{center}
  \fcolorbox{blue}{verysoftblue}{\begin{minipage}{0.9\linewidth}\centering
    \begin{align*}
      f=\displaystyle\sum_{\ell\geq0}f_\ell &\xmapsto{\quad \cG\quad} \displaystyle\sum_{\ell\geq0}\lambda_\ell f_\ell\\[1.5ex]
      f=\displaystyle\sum_{\ell\geq0}f_\ell &\xmapsto{\quad \cF\quad} \displaystyle\sum_{\ell\geq0}\lambda'_\ell f_\ell. 
    \end{align*}
  \end{minipage}}
\end{center}
...
\vspace{-0.4em}
\[ \{Y_{\ell m}:-\ell\leq m\leq\ell\}. \vspace{-0.5em} \]
...
On the equator $\theta=\pi/2$, so $\cos\theta=0$. Thus $P_\ell'(0)$ describes how $Y_{\ell0}$ changes in the direction normal to the equator, which is exactly the quantity appearing in the eigenvalues of the $\cG$-transform.
}
...
\vspace{-0.5em}
\[ \cG[Y_{\ell0}](e_3) = \lambda_\ell Y_{\ell0}(e_3), \vspace{-0.5em} \]
...
\[ \ker\cG=\Cinf_{\even}(\Sph^2), \quad \cG:\Cinf_{\odd}(\Sph^2)\rightarrow \Cinf_{\odd}(\Sph^2) \text{ is invertible}. \]
}
...
Thus, on the odd subspace, $\cG$ has Sobolev scaling of an operator of order $1/2$.
...
\medskip
Therefore, $\cG N=M$ gains half a derivative, $N$ is $1/2$ derivative smoother than $M$. Without imposing oddness, $N$ is determined only up to an arbitrary even function, , since it lies in $\ker\cG$.
}
